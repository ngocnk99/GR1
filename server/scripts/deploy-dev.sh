...
DOCKER_IMAGE=10.0.200.107:5000/vietnamtickets-api-"$BUILD_TAG":"$BUILD_NUMBER"
ENV_FILE=/home/env/apivietnamtickets/.env.production # Default to dev environment
CONTAINER_NAME=vietnamtickets-api-"$BUILD_TAG"

if [ "$BUILD_TAG" = "master" ]; then
  ENV_FILE=/home/env/apivietnamtickets/.env.production # Production environment file
elif [ "$BUILD_TAG" = "test" ]; then
  ENV_FILE=/home/env/apivietnamtickets/.env.production # Staging environment file
fi
